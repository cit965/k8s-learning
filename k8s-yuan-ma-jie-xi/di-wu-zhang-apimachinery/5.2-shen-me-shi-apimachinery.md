...
* staging/src/k8s.io/apiserver/pkg/storage/etcd3: 包含了与 etcd 交互的具体实现。这里定义了如何将 Kubernetes 资源对象存储到 etcd 中，以及如何从 etcd 中检索这些对象。
* staging/src/k8s.io/apiserver/pkg/storage: 定义了存储接口和抽象，etcd 是其中的一个具体实现。
* staging/src/k8s.io/apiserver/pkg/server/options: 包含了与 etcd 相关的配置选项，如 etcd 的连接地址、证书配置等。
