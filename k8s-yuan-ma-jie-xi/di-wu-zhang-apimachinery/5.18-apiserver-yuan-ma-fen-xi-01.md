...
<pre class="language-go"><code class="lang-go">// CreateServerChain creates the apiservers connected via delegation.
...
func CreateAggregatorServer(aggregatorConfig aggregatorapiserver.CompletedConfig, delegateAPIServer genericapiserver.DelegationTarget, crds apiextensionsinformers.CustomResourceDefinitionInformer, crdAPIEnabled bool, apiVersionPriorities map[schema.GroupVersion]APIServicePriority) (*aggregatorapiserver.APIAggregator, error) {
	...
	// 3. CRD 注册控制器
	// 通过 apiExtensionInformers 监听集群中 CRD 的创建、更新、删除事件
	// 将新创建的 CRD 自动注册到 API Server 的 API 发现机制中
	//  当你执行 kubectl apply -f my-crd.yaml 创建一个 CRD 时：
<strong>  //       - CRD 对象被创建到 etcd
</strong>  //       - 这个控制器检测到新 CRD
  //       - 自动将其注册到 API 发现系统
  //       - 之后你就可以使用 kubectl get &#x3C;custom-resource> 操作自定义资源了
	var crdRegistrationController controller  // 声明控制器变量
	
	if crdAPIEnabled {  // 检查 CRD 功能是否启用
	    crdRegistrationController = crdregistration.NewCRDRegistrationController(
	        // 参数1: CRD Informer - 监听 CRD 资源变化
	        apiExtensionInformers.Apiextensions().V1().CustomResourceDefinitions(),
	        
	        // 参数2: 自动注册控制器 - 负责将 API 注册到发现机制
...
请求进来
  ...
  │   │
  │   ├─ RootPath == "/apis"?
  │   │   └─ Yes → path 精确匹配 "/apis" 或 "/apis/"?
  │   │       ├─ Yes → goRestfulContainer ✅
  │   │       └─ No → 继续
  │   │
  │   └─ 前缀匹配 RootPath?
  │       └─ Yes → 边界检查通过?
...
```
请求: GET /apis/apps/v1/deployments
↓
遍历 ws1: RootPath="/apis"
  → case ws.RootPath() == "/apis": ✅
  → if path == "/apis": ❌ (不是精确匹配)
  → 继续检查下一个 case
  ...
  → 边界检查: path[5] == '/' ✅
  → 分发到 goRestfulContainer
  → 内部路由到 apps/v1 的 Deployment handler

```

#### 示例 2: `/apis/apps/v1/deployments`
...
遍历所有 WebServices
  → 没有匹配的 RootPath
↓
fallback 到 nonGoRestfulMux
↓
nonGoRestfulMux 处理健康检查
...
```go
// 注释中的解释
// We can't rely on a prefix match since /apis matches everything
```

因为 `/apis` 是所有 API 组的公共前缀，如果只用前缀匹配会导致：

* 无法区分 API 发现请求和具体资源请求
* 路由逻辑混乱
...
确保只匹配真正的子路径，而不是碰巧有相同前缀的其他路径。

#### 3. 为什么直接调用 Dispatch 而不是 ServeHTTP？

```go
// 注释中的解释
// don't use servemux here because gorestful servemuxes get messed up 
// when removing webservices
```

这是为了避免 go-restful 在动态添加/删除 WebService 时的并发问题。
