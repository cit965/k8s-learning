...
cheme 起到了一个类型（Type）注册中心的作用，在API Server内部，全局只有一个Scheme实例，各个版本的API资源，会将他们的类型，注册到 scheme中 s来，同时，也会将如何进行类型转换的方法注册到scheme中来，后续在 Handler 中进行版本转换以及序列化时，则会使用 scheme中注册的类型创建对应版本的对象，以及使用注册的类型转换的方法对不同版本的对象进行转换。
